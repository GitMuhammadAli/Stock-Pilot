...
export function OrderItemProvider({ children }: OrderItemProviderProps) {
  ...
  // Get all order items (GET /api/orderitem)
  const getAllOrderItems = useCallback(async () => {
    ...
    try {
      ...
      const result: { success: boolean; data?: OrderItem[]; message?: string } =
        await response.json();
      ...
      setError(
        "An error occurred while fetching all order items: " + err.message
      );
      console.error(err);
    } finally {
      setLoading(false);
  ...
  // Get order items for a specific order (GET /api/orderitem/order/{orderId})
  const getAllOrderItemsForOrder = useCallback(async (orderId: string) => {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(`/api/orderitem/order/${orderId}`, {
        method: "GET",
        headers: getAuthHeaders(),
      });
      const result: { success: boolean; data?: OrderItem[]; message?: string } =
      ...
      if (result.success && result.data) {
        ...
        setError(
          result.message || `Failed to fetch order items for order ${orderId}`
        );
      }
    } catch (err: any) {
      setError(
        `An error occurred while fetching order items for order ${orderId}: ` +
          err.message
      );
      console.error(err);
    } finally {
  ...
  // Get order items for a specific product (GET /api/orderitem/product/{productId})
  const getAllOrderItemsForProduct = useCallback(async (productId: string) => {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(`/api/orderitem/product/${productId}`, {
        method: "GET",
        headers: getAuthHeaders(),
      });
      const result: { success: boolean; data?: OrderItem[]; message?: string } =
      ...
      if (result.success && result.data) {
        ...
        setError(
          result.message ||
            `Failed to fetch order items for product ${productId}`
        );
      }
    } catch (err: any) {
      setError(
        `An error occurred while fetching order items for product ${productId}: ` +
          err.message
      );
      console.error(err);
    } finally {
  ...
  // Get a single order item by ID (GET /api/orderitem/{id})
  const getOrderItem = useCallback(
    async (id: string): Promise<OrderItem | null> => {
      setLoading(true);
      setError(null);
      try {
        const response = await fetch(`/api/orderitem/${id}`, {
          method: "GET",
          headers: getAuthHeaders(),
        });
        const result: { success: boolean; data?: OrderItem; message?: string } =
        ...
        setError(
          "An error occurred while fetching the order item: " + err.message
        );
        console.error(err);
        return null;
      } finally {
  ...
  // Get total for a specific order (GET /api/orderitem/order/{orderId}/total)
  const getOrderItemsTotal = useCallback(
    async (orderId: string): Promise<number | null> => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/orderitem/order/${orderId}/total`, {
          method: "GET",
          headers: getAuthHeaders(),
        });
        const result: { success: boolean; total?: number; message?: string } =
          await response.json();
        ...
        if (result.success && typeof result.total === "number") {
          ...
          setError(
            result.message || `Failed to fetch total for order ${orderId}`
          );
        ...
        setError(
          `An error occurred while fetching order total for order ${orderId}: ` +
            err.message
        );
        console.error(err);
        setOrderTotal(0);
  ...
  // Create a new order item (POST /api/orderitem)
  const createOrderItem = useCallback(
    async (data: CreateOrderItemData): Promise<boolean> => {
      setLoading(true);
      setError(null);
      try {
        const response = await fetch("/api/orderitem", {
        ...
        if (result.success && result.data) {
          ...
          if (data.orderId === selectedOrderItem?.orderId) {
            // Check if new item is for the same order
        ...
        setError(
          "An error occurred while creating the order item: " + err.message
        );
        console.error(err);
        return false;
      } finally {
  ...
  // Update an existing order item (PUT /api/orderitem/{id})
  const updateOrderItem = useCallback(
    async (id: string, data: UpdateOrderItemData): Promise<boolean> => {
      setLoading(true);
      setError(null);
      try {
        const response = await fetch(`/api/orderitem/${id}`, {
        ...
        if (result.success && result.data) {
          ...
          if (result.data.orderId) {
            // Assuming orderId is returned in updated data
        ...
        setError(
          "An error occurred while updating the order item: " + err.message
        );
        console.error(err);
        return false;
      } finally {
  ...
  // Delete an order item (DELETE /api/orderitem/{id})
  const deleteOrderItem = useCallback(
    async (id: string): Promise<boolean> => {
      ...
      try {
        ...
        const result: { success: boolean; message?: string; data?: OrderItem } =
          await response.json(); // Backend might return deleted item
        ...
        if (result.success) {
          ...
          if (result.data?.orderId) {
            // Check if backend returned the deleted item data
            getOrderItemsTotal(result.data.orderId);
          } else if (orderItems.find((item) => item.id === id)?.orderId) {
            // Fallback if backend doesn't return deleted item
            getOrderItemsTotal(
              orderItems.find((item) => item.id === id)?.orderId as string
            );
          }
          return true;
        } else {
        ...
        setError(
          "An error occurred while deleting the order item: " + err.message
        );
        console.error(err);
        return false;
      } finally {
    ...
    [selectedOrderItem, orderItems, getOrderItemsTotal]
  ); // Add orderItems as dependency if using find on it
